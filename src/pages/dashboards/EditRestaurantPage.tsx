// src/pages/dashboards/EditRestaurantPage.tsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { Building, MapPin, Phone, ChefHat, DollarSign, Clock, X, Save } from 'lucide-react';
...
export const EditRestaurantPage: React.FC = () => {
  ...
  const loadRestaurantData = async () => {
    try {
      ...
      setFormData({
        ...
        delivery_fee: restaurant.delivery_fee || 100,
        estimated_delivery_time: restaurant.estimated_delivery_time || 30,
        is_active: restaurant.is_active ? 'true' : 'false',
      });
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await RestaurantOwnerService.updateRestaurant(parseInt(id!), formData);
